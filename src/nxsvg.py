...
class SVGRenderer(object):
    ...
    def draw(self, g, pos=None,
        ...
        dwg.viewbox(minx=-self.Margin * aspect, miny=-self.Margin, width=aspect *
                    self.GlobalScale + 2 * self.Margin * aspect, height=self.GlobalScale + 2 * self.Margin)
        dwg.fit()
        label_layer = Group()
        node_layer = Group()
        edge_layer = Group()
